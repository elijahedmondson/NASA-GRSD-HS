...
get.effect.size2(pheno = pheno, pheno.col = "albino", probs = probs, sdp.file = "~/Desktop/R/QTL/WD/HS_Sanger_SDPs.txt.bgz",
...
get.effect.size2 = function(pheno = pheno, pheno.col, probs = probs, sdp.file = "~/Desktop/R/QTL/WD/HS_Sanger_SDPs.txt.bgz",
                           markers, threshold = 5.05, dir = "/Users/elijah/Desktop/R/QTL/WD/2.\ Binomial\ Mapping/")
{
        library(Rsamtools)
        ...
        for(j in 1:length(files)){
                ...
                for(i in 1:19) {
                        ...
                        SNP1 = qtli$start[which.min(qtli$p.value)]
                        LOD1 = -log10(qtli$p.value[which(qtli$start == SNP1)])
                        ...
                        if(LOD1 > threshold) {
                                ...
                                EFFECT[i,] = c(pheno.col, chr, SNP1, LOD1, odds[3], oddsCI[3], oddsCI[6], ANOVA$`Pr(>Chi)`[2],
                                               mod1$aic, R2$CoxSnell, R2$Nagelkerke, R2$McFadden, R2$Tjur, R2$sqPearson, D2)
                                print(EFFECT)
                                write.csv(EFFECT, file = paste0(files[j], "QTL", ".csv"))
                                rm(qtl, LOD, oddsCI, ANOVA, mod1, mod0, R2, D2, SNP)
                        }
