...
define.QTL(dir = "/Users/elijah/Desktop/R/QTL/WD/2.\ Binomial\ Mapping/Rdata/", threshold = 5.05)
...
define.QTL = function(dir = "~/Desktop/R/QTL/WD/2.\ Binomial\ Mapping/Rdata/", threshold = 5.8) {
        ...
        for(j in 1:length(files)){
                ...
                for(i in 1:19) {
                        ...
                        if(top > threshold) {
                                LOD.drop.int = top
                                max.LOD.peak <- qtl[[i]]@ranges[which(-log10(qtl[[i]]$p.value) == min(LOD.drop.int))]
